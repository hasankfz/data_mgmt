...
FROM dbo.[TecDoc.Articles.Articles] td_art 
  -- Links to articles for passenger cars (PKW or PC)
     LEFT OUTER JOIN dbo.[TecDoc.Linkages.PassengerCars] td_pcl ON td_pcl.[Article:Link] = td_art.[ArticleNo]
  -- Links to passenger cars (PKW or PC)
     LEFT OUTER JOIN dbo.[TecDoc.LinkingTargets.PassengerCars] td_pc ON td_pc.[PassengerCarNo] = td_pcl.[LinkingTarget:Link] 
  ...
  -- Generic Articles
     LEFT OUTER JOIN dbo.[TecDoc.ProductClassification.GenericArticles] td_ga ON td_ga.[GenericArticleNo] = td_pcl.[GenericArticle:Link]
     LEFT OUTER JOIN dbo.[TecDoc.ProductClassification.GenericArticles <TecDoc.GeneralData.UsedLanguages>] td_ga_de ON td_ga_de.[:Id] = td_ga.[:Id]
              AND td_ga_de.[:TecDoc.GeneralData.UsedLanguages_Id] = '3e8124e1-b1cd-4faf-bdda-eebbfaf0acfd' -- DE
  -- Assembly Group
     LEFT OUTER JOIN dbo.[TecDoc.ProductClassification.AssemblyGroups] td_ag ON td_ag.[AssemblyGroupNo] = td_ga.[AssemblyGroup:Link]
     LEFT OUTER JOIN dbo.[TecDoc.ProductClassification.AssemblyGroups <TecDoc.GeneralData.UsedLanguages>] td_ag_de ON td_ag_de.[:Id] = td_ag.[:Id] 
                 AND td_ag_de.[:TecDoc.GeneralData.UsedLanguages_Id] = '3e8124e1-b1cd-4faf-bdda-eebbfaf0acfd' -- DE
  -- Data Supplier
     LEFT OUTER JOIN dbo.[TecDoc.GeneralData.DataSupplier] td_ds ON td_ds.[DataSupplierNo] = td_art.[DataSupplier:Link]
